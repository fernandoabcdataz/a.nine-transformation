...
WITH items_raw AS (
    SELECT
        ...
        JSON_VALUE(data, '$.SalesDetails.AccountCode') AS sales_detail_account_code,
        JSON_VALUE(data, '$.SalesDetails.TaxType') AS sales_detail_tax_type,
        CAST(JSON_VALUE(data, '$.SalesDetails.UnitPrice') AS NUMERIC) AS sales_detail_unit_price,
        JSON_VALUE(data, '$.PurchaseDetails.AccountCode') AS purchase_detail_account_code,
        JSON_VALUE(data, '$.PurchaseDetails.TaxType') AS purchase_detail_tax_type,
        CAST(JSON_VALUE(data, '$.PurchaseDetails.UnitPrice') AS NUMERIC) AS purchase_detail_unit_price
    FROM 
